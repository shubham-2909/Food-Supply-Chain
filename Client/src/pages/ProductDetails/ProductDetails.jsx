...
const ProductDetails = () => {
  ...
  const getProductDetails = async () => {
    ...
    console.log("product details here ", productResponse.data);
  ...
  const handleBuy = async () => {
    ...
    navigate(
      role == Constants.ROLE[2] ? `/${role}/purchases` : `/${role}/inventory`
    );
  };
    ...
    <Container>
      ...
      >
        <Grid item md={4} sm={6}>
          <Card style={{ boxShadow: "0 0 10px 0" }}>
            <CardMedia
              ...
              height="300"
              width="auto"
              image={`${product.details.ipfs_hash}`}
        ...
        <Grid item md={8} sm={12}>
          ...
          <p>
            <b>Product Name:</b> {product.details.name}
          </p>
          <p>
            <b>Price:</b> {product.details.price} ETH
          </p>
          <p>
            <b>Stage:</b>{" "}
            {Constants.STAGE[product.details.stage].charAt(0).toUpperCase() +
              Constants.STAGE[product.details.stage].slice(1)}
          </p>
          {product.details.currentOwner.toLowerCase() != account && (
            <Button
              variant="contained"
              color="lightOrange"
              onClick={handleBuy}
